...
GenerateScriptRenderingIcons <- function(
  ...
  # Prevent icons from being suspended when their collapse panel is hidden.
  # Without this, icons only render after the first reactive fires following
  # panel expansion, causing a visible delay when a group is first opened.
  cat("local({\n")
  cat("  iconIds <- c(", paste0('"', paste0(prefixGuiId, ids, "icon"), '"', collapse = ", "), ")\n")
  cat("  for (id in iconIds) outputOptions(", outputObjectName, ", id, suspendWhenHidden = FALSE)\n", sep = "")
  cat("})\n")
  sink()
